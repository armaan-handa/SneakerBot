...
NIKE_HOME_URL = "https://www.nike.com/in/"
LOGGER = logging.getLogger()


def run(driver, username, password, url, shoe_size=10, login_time=None, release_time=None,
...
def login(driver, username, password):
    ...
    wait_until_clickable(driver=driver, xpath="//button[@data-var='loginBtn']")

    LOGGER.info("Clicking login button")
    driver.find_element_by_xpath("//button[@data-var='loginBtn']").click()
    ...
    while True:
        ...
        time.sleep(3)
        if driver.find_element_by_xpath("//*[@id='AccountMenu']"):
            break
        elif driver.find_element_by_xpath("//button[@value='Dismiss this error']"):
            driver.find_element_by_xpath("//button[@value='Dismiss this error']").click()

    LOGGER.info("Successfully logged in")


def select_shoe_size(driver, shoe_size):
    LOGGER.info("Waiting for sizes to become clickable")
    wait_until_clickable(driver, xpath="//input[@name='skuAndSize']")
    
    LOGGER.info("Getting list of shoe sizes")
    size_list = driver.find_elements_by_xpath("//input[@name='skuAndSize']")

    for size in size_list:
        size_val = size.get_attribute('value')
        size_val = size_val.split(":")[1]
        if size_val == shoe_size:
            try:
                parent = size.find_element_by_xpath("./..")
                parent.click()
                LOGGER.info("Selected size: " + size_val)
            except:
                LOGGER.info("Could not select size")


def add_shoe_to_bag(driver):
    ...
    LOGGER.info("Adding shoe to bag")
